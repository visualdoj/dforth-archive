...
begin
  ...
  end else if TUInt(FMachine.RB) = TUInt(FMachine.RP) then begin
    //FMachine.EC := Length(FMachine.FEmbro)
    Machine.RB := Machine.ROP;
    // Machine.FState := Machine.ROI;
    FMachine.FRunning := False;
  end else begin
    Machine.LogError('[exit] R stack error (' +
                       IntToStr(TUInt(FMachine.RP) - TUInt(FMachine.RB)) + ')');
...
procedure TEmbroCommands.q_compile_q(Machine: TForthMachine; Command: PForthCommand);
begin
  Machine.EWO(Machine.C[Machine.ERU].Name);
end;
...
procedure TForthMachine._vocabulary(Machine: TForthMachine; Command: PForthCommand);
var
  V: PVoc;
begin
  New(V);
...
procedure TForthMachine._vLOCAL(Machine: TForthMachine; Command: PForthCommand);
begin 
  WUP(vLOCAL);
end;

procedure TForthMachine._vGLOBAL(Machine: TForthMachine; Command: PForthCommand);
begin 
  WUP(vGLOBAL);
end;

procedure TForthMachine._TargetPush(Machine: TForthMachine; Command: PForthCommand);
begin
  TargetPush(WOP);
end;

procedure TForthMachine._TargetPop(Machine: TForthMachine; Command: PForthCommand);
begin
  WUP(TargetPop);
end;

procedure TForthMachine._ContextPush(Machine: TForthMachine; Command: PForthCommand);
begin
  ContextPush(WOP);
end;

procedure TForthMachine._ContextPop(Machine: TForthMachine; Command: PForthCommand);
begin
  WUP(ContextPop);
end;
...
{$IFNDEF FLAG_FPC}{$REGION 'embro commands'}{$ENDIF}
  ...
  AddCommand('(compile)', FEmbroCommands.q_compile_q, True);
...
{$IFNDEF FLAG_FPC}{$REGION 'string commands'}{$ENDIF}
  ...
  AddCommand('str#', FStringCommands.str_len);
  AddCommand('str=', FStringCommands.str_equel);
  AddCommand('str0', FStringCommands.str_nil);
  AddCommand('str+', FStringCommands.str_concat);
...
procedure TForthMachine._FIND_(Machine: TForthMachine; Command: PForthCommand);
var
  S: TString;
  I: Integer;
  Item: PVocItem;
begin
  ...
  for I := High(Context) downto 0 do begin
    ...
    while Item <> nil do begin
      if TString(C[Item^.Index]^.Name) = S then begin
  ...
  // Writeln('_FIND_(', S, ')');
  //for I := High(C) downto 0 do
  //  if TString(C[I].Name) = S then begin
  //    WUI(I);
  //    WUI(0);
  //    // Writeln('_FIND_(', S, ') done');
  //    Exit;
  //  end;
  WUI(-1);
  // Writeln('_FIND_(', S, ') failed');
end;

procedure TForthMachine._NOTFOUND_(Machine: TForthMachine; Command: PForthCommand);
var
  S: TString;
  I: Integer;
begin
  S := WOS;
  // Writeln('_NOTFOUND_(', S, ')');
  if ConvertStrToInt(S, I) = 0 then begin
    WUI(I);
    WUI(BOOL_FALSE);
    Exit;
  end;
