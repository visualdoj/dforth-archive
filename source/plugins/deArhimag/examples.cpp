...
// явл€етс€ ли чанк данными
int IsChunkData(const chunk_t& chunk)
{
  return (chunk.opcode == 0 && chunk.len != 0);
}

// ¬ычисление размера, занимаемого чанком в шитом коде
//   ¬ случае, если это чанк вызова команды, нужно ещЄ учесть место,
...
// ¬ычисление номера байта (embroptr) начала чанка с номером index
// ¬ернЄт -1, если index не €вл€етс€ индексом никакого чанка
...
// ¬ычисление чанка, в который ссылаетс€ данный embroptr
//     code [in]
//       шитый код, в котором ищетс€ чанк
//     ref [in]
